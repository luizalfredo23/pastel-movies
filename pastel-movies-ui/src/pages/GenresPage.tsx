...
import { createGenre, deleteGenre, fetchGenresPage, updateGenre } from '../api/genresApi'
...
export function GenresPage() {
  ...
  const [data, setData] = useState<SpringPage<Genre> | null>(null)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setError(null)
      try {
        const res = await fetchGenresPage({
          page,
          size,
  ...
  const save = async () => {
    try {
      setError(null)
      if (editing) await updateGenre(editing.id, { name: formName.trim() })
      else await createGenre({ name: formName.trim() })
      setModalOpen(false)
      await refresh()
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-ink">Genres</h2>
          <p className="text-sm text-ink-muted">Browse and manage genres</p>
        </div>
        <button
          type="button"
          onClick={openCreate}
      ...
      <div className="overflow-x-auto rounded-2xl ring-1 ring-pastel-purple-deep/20">
        <table className="min-w-full border-collapse text-left text-sm">
          <thead className="bg-pastel-yellow/50 text-ink">
            <tr>
          ...
          <tbody>
            {loading ? (
              ...
              data?.content.map((g) => (
                <tr key={g.id} className="border-t border-pastel-purple-deep/15 odd:bg-white/70">
                  <td className="px-4 py-3 text-ink-muted">{g.id}</td>
