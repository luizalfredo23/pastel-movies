...
import { createActor, deleteActor, fetchActorsPage, updateActor } from '../api/actorsApi'
...
export function ActorsPage() {
  ...
  const [data, setData] = useState<SpringPage<Actor> | null>(null)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setError(null)
      try {
        const res = await fetchActorsPage({
          page,
          size,
  ...
  const save = async () => {
    try {
      setError(null)
      if (editing) await updateActor(editing.id, { name: formName.trim() })
      else await createActor({ name: formName.trim() })
      setModalOpen(false)
      await refresh()
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-ink">Actors</h2>
          <p className="text-sm text-ink-muted">Browse and manage actors</p>
        </div>
        <button
          type="button"
          onClick={openCreate}
      ...
      <div className="overflow-x-auto rounded-2xl ring-1 ring-pastel-purple-deep/20">
        <table className="min-w-full border-collapse text-left text-sm">
          <thead className="bg-pastel-blue/40 text-ink">
            <tr>
          ...
          <tbody>
            {loading ? (
              ...
              data?.content.map((a) => (
                <tr key={a.id} className="border-t border-pastel-purple-deep/15 odd:bg-white/70">
                  <td className="px-4 py-3 text-ink-muted">{a.id}</td>
